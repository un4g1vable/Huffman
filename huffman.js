...
function decode(codes, text) { // функции подаётся массив массивов формата ['код' , 'буква'] и двоичное сообщение
    ...
    for (let i = 0; i <= text.length; i++) {
        if (codes.get(code)) // если нашёлся такой код из таблицы
            ...
            result += ""; //иначе ничего в результат не складвыаем
        if (codes.get(code))
            code = text[i]; //берём цифру из сообщения
        else
            code = code + text[i]; //наращиваем, прибавляем ещё цифру
    }
...
if (arg[2] === "code"){
    ...
    let encoded = fs.readFileSync(arg[3], 'utf8'); //запускаем программу с чтения файла
    let table = fs.readFileSync(arg[4], 'utf8'); //запускаем программу с чтения файла
